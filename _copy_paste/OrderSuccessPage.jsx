...
import { CheckCircle, Clock, Phone, ArrowRight, MapPin, Star, Sparkles, Bell } from "lucide-react";
...
export default function OrderSuccessPage() {
    ...
    const enableAlerts = async () => {
        const state = await ensurePushSubscription({ silent: false });
        setPushState(state);
        if (state === "subscribed") toast.success("You'll get a notification when your order status changes.");
        else if (state === "denied") toast.error("Notifications were blocked. Enable them in your browser settings to get order updates.");
        else if (state === "unsupported") toast.error("Your browser doesn't support order alerts.");
    };
        ...
        <div className="max-w-2xl mx-auto px-4 py-16 md:py-24 text-center" data-testid="order-success-page">
            ...
            <div className="flex gap-3 justify-center flex-wrap">
                ...
                {/* Subtle one-tap "enable order alerts" — the natural moment to ask.
                    Hidden when push isn't supported / already granted. */}
                {pushAvailable && pushState !== "granted" && pushState !== "denied" && (
                    <button
                        type="button"
                        onClick={enableAlerts}
                        data-testid="success-enable-alerts"
                        className="inline-flex items-center gap-2 bg-brand-yellow hover:bg-brand-yellow/90 text-brand-ink rounded-full px-5 py-3 font-semibold text-sm transition-colors"
                    >
                        <Bell className="w-4 h-4" /> Enable order alerts
                    </button>
                )}
