...
async def update_online_order_status(order_id: str, body: OnlineOrderStatusUpdate, request: Request):
    ...
    try:
        ...
        if order and body.status in {"accepted", "preparing", "out_for_delivery", "delivered", "cancelled", "rejected"}:
...
async def public_restaurant_info():
    """Public: minimal restaurant info needed to render the receipt QR codes
    and the customer-facing review page."""
    ...
    return {
        "name": g.get("restaurant_name", "Karachi Naseeb Biryani & Murg Pulao"),
        "phone": g.get("restaurant_phone", "+923004928411"),
        "address": g.get("restaurant_address", "68 Chatri Chowk, Punjab Small Industry, D Block, Lahore"),
