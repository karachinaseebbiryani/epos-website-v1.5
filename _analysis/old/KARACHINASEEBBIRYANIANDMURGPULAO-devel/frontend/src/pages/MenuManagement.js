...
import {
  Plus, Pencil, Trash2, FolderPlus, UtensilsCrossed, Package, GripVertical, Lock,
} from "lucide-react";
...
export default function MenuManagement() {
  ...
  const [itemForm, setItemForm] = useState({ name: "", price: "", price_fp1: "", price_fp2: "", category_id: "", stock: "100", low_stock_threshold: "10", color: null });
  ...
  const saveItem = async () => {
    ...
    try {
      ...
      setItemDialog(false); setItemForm({ name: "", price: "", price_fp1: "", price_fp2: "", category_id: "", stock: "100", low_stock_threshold: "10", color: null }); setEditingItem(null); fetchData();
    } catch (err) { toast.error(err.response?.data?.detail || "Failed"); }
  };
      ...
      <Tabs defaultValue="items" className="space-y-4">
        ...
        <TabsContent value="items">
          <div className="flex justify-end mb-4">
            <Button data-testid="add-item-btn" disabled={!canEdit}
              onClick={() => { setEditingItem(null); setItemForm({ name: "", price: "", price_fp1: "", price_fp2: "", category_id: "", stock: "100", low_stock_threshold: "10", color: null }); setItemDialog(true); }}
          ...
          {menuItems.length === 0 ? (
            <Card className="border-[#E5E2DC]"><CardContent className="flex flex-col items-center py-16">
              ...
              <p className="text-sm" style={{ color: "#5C5F5C" }}>No menu items yet. Create categories first, then add items.</p>
            </CardContent></Card>
          ) : (
